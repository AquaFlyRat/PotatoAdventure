...
namespace Utils
{
    ...
    template <typename T> class NestedVector
    {
      public:
        using value_type = T;
        using size_type = int;
      ...
      private:
        ...
        std::vector<Element> data;
        size_type sub_array_size;

      public:
        constexpr NestedVector() : sub_array_size(256) {}
        NestedVector(size_type main_size, size_type sub_size) : data(main_size)
        {
            sub_array_size = sub_size;
        }

        void alloc(size_type main_size, size_type sub_size)
        {
            free();
            data.resize(main_size);
            sub_array_size = sub_size;
        }
        void free()
        {
        ...
        {
            const auto ref = data[index / sub_array_size];
            if (ref.size == 0)
                return 0;
            return ref[index % sub_array_size].exists;
        }

        T &add(size_type index)
        {
            auto ref = data[index / sub_array_size];
            if (ref.size == 0)
                ref.data.resize(sub_array_size);
            auto element_ref = ref.data[index % sub_array_size];
        ...
        template <typename I> [[nodiscard]]       T &operator[](I index)       {return data[index / sub_array_size].data[index % sub_array_size].object;}
        template <typename I> [[nodiscard]] const T &operator[](I index) const {return data[index / sub_array_size].data[index % sub_array_size].object;}
    };
